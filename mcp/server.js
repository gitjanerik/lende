// MCP-server (PoC) for Lende — eksponerer kart-pipelinen og
...
const GEOCODE_UA = 'lende-mcp/0.1 (turkart-generator)'
...
const server = new McpServer({ name: 'lende', version: '0.1.0' })
...
server.registerTool(
  ...
  async ({ ruteIndeks, navn, filsti }) => {
    ...
    const slug = navn.replace(/[^a-z0-9æøå]+/gi, '-').toLowerCase()
    const gpxPath = resolve(filsti ?? resolve(tmpdir(), 'lende-mcp', `${slug}.gpx`))
...
console.error('lende MCP-server kjører (stdio)')
