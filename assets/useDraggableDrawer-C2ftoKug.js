import{$ as e,B as t,D as n,et as r}from"./symbolizer-BpW995tk.js";import{Q as i}from"./useSearchKeyboard-DYkbXBKW.js";function a(e){return String(e).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&apos;`})[e])}function o(e,t,n=`Lende tur`){if(!e?.points?.length)return``;let r=new Date(e.opprettet??Date.now()).toISOString(),o=a(e.navn||`Tur `+new Date(e.opprettet??Date.now()).toLocaleString(`no-NO`)),s=e.points.map(e=>{let n=i(e.x,e.y,t),r=new Date(e.t).toISOString(),a=e.accM==null?``:`\n        <hdop>${(e.accM/5).toFixed(1)}</hdop>`;return`      <trkpt lat="${n.lat.toFixed(7)}" lon="${n.lon.toFixed(7)}">
        <time>${r}</time>${a}
      </trkpt>`}).join(`
