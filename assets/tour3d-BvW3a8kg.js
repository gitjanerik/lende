...
void main() {
    ...
    `}),r=new I(t,n);return r.frustumCulled=!1,r.renderOrder=-1,{mesh:r,geometry:t,material:n,setNight(e){n.uniforms.uZenith.value.set(e?So:yo),n.uniforms.uHorizon.value.set(e?Co:bo)}}}function Eo(e){return new Ie(new q(xo),e*.6,e*2.6)}var Do=1.6,Oo=.055;function ko({radius:e=25e3,avstand:t=null}={}){let n=t??e*.82,r=new nt(1,1),i=new Ge({transparent:!0,depthWrite:!1,side:2,uniforms:{uLysAndel:{value:.75},uLysside:{value:0},uFarge:{value:new q(`#fdf6df`)},uJordskinn:{value:Oo}},vertexShader:`
      varying vec2 vUv;
      void main() {
        vUv = uv;
        gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
    ...
    `}),a=new I(r,i),o=2*n*Math.tan(Do*Math.PI/180/2);a.scale.setScalar(o),a.frustumCulled=!1;let s=new M;return s.add(a),{group:s,mesh:a,geometries:[r],materials:[i],sett(e){if(!e||!Number.isFinite(e.azimut)||!Number.isFinite(e.hoyde))return;let[t,r,o]=_o(e.azimut,e.hoyde,n);a.position.set(t,r,o),i.uniforms.uLysAndel.value=Math.max(0,Math.min(1,e.lysAndel??.75)),i.uniforms.uLysside.value=Number.isFinite(e.lyssideVinkel)?e.lyssideVinkel:0,a.visible=e.hoyde>-2*Math.PI/180},update(e){e&&a.quaternion.copy(e.quaternion)},dispose(){r.dispose(),i.dispose()}}}function Ao(e){return Math.max(1.1,Math.min(5,4.6-.62*e))}function jo(e){return Math.max(.32,Math.min(1,1.05-.115*e))}var Mo=-1*Math.PI/180,No=.13;function Po({radius:e=25e3,lat:t=null,lon:n=null,dato:r=null,starCount:i=160}={}){let a=new M;a.visible=!1;let o=e*.9,s=Number.isFinite(t)&&Number.isFinite(n),c=[],l=[],u=[],d=new Map;if(s){let e=r??new Date,i=no(e,n);for(let n=0;n<Ga.length;n++){let r=Ga[n],a=mo(r.ra,r.dek,e),{azimut:s,hoyde:f}=ho(a.ra,a.dek,i,t);if(f<Mo)continue;let[p,m,h]=_o(s,f,o);d.set(n,c.length/3),c.push(p,m,h),l.push(Ao(r.mag)),u.push(jo(r.mag))}}else{let e=Fo(1337);for(let t=0;t<i;t++){let t=e()*Math.PI*2,n=.12+e()*.88,r=Math.sqrt(Math.max(0,1-n*n));c.push(Math.cos(t)*r*o,n*o,Math.sin(t)*r*o),l.push(1.6+e()*1.6),u.push(.55+e()*.4)}}let f=new Ue;f.setAttribute(`position`,new K(new Float32Array(c),3)),f.setAttribute(`storrelse`,new K(new Float32Array(l),1)),f.setAttribute(`styrke`,new K(new Float32Array(u),1));let p=new Ge({transparent:!0,depthWrite:!1,blending:2,uniforms:{uFarge:{value:new q(`#ffeec4`)}},vertexShader:`
