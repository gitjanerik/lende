...
# Lende — PreToolUse-vakt for CHANGELOG-disiplin.
#
...
# src/version.js er STAGET (= versjon bumpet) uten at CHANGELOG.md også er
...
printf '%s\n' "$staged" | grep -qx 'src/version.js' && bumped=1 || bumped=0
printf '%s\n' "$staged" | grep -qx 'CHANGELOG.md' && changelog=1 || changelog=0

if [ "$bumped" = "1" ] && [ "$changelog" = "0" ]; then
  {
    echo "BLOKKERT: src/version.js er bumpet, men CHANGELOG.md er ikke med i denne committen."
    echo ""
