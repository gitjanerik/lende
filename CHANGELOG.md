...
## 2026-09-24 — v7.9.19: Kartstilen «Vinter», belyste og ubelyste løyper, Turrutebasen-skiløyper og Skisporet-lenke

Ny kartstil «Vinter»: et print-aktig ark med mye hvitt, blekt vann og dempet skog, der ski- og lysløyper er røde og synlige som standard sammen med slalåmbakker og heistraséer. Løypene tegnes også over vann, som de ofte går om vinteren. Skiløype-symbolet skiller nå belyst fra ubelyst: prikkene er lyset, så en løype uten `lit`/`piste:lit=yes` er linja alene, og laget heter «Ski- og lysløyper». I tillegg hentes Turrutebasens `Skiløype`-type (med belysning fra kilden) som en sekundærkilde på restbudsjettet, i appen og i headless, og strekk som OSM alt har som skiløype tas bort. Infopanelet har fått en «Skisporet»-knapp ved siden av UT.no og Google Maps som åpner omtrent samme kartutsnitt for live føremelding. Lende-chatten har fått en egen instruks om skiløyper, slik den har for sykkelruter: «vis skiløypene» slår på laget, «jeg skal på ski» bytter til Vinter, og den forstår «skiløyper», «langrenn», «skiheis» og «slalåm» som lagnavn. Den forklarer at prikkene betyr lys, og sender spørsmål om føre videre til Skisporet-knappen i stedet for å gjette. MCP-verktøyet `juster_kart` har fått samme veiledning, både i stdio-serveren og i Workeren, fra én delt tekst i `kartStiler.js`. Laget er fortsatt av som standard i alle andre kartstiler, og kart må bygges på nytt.
...
## 2026-09-24 — v7.9.13: Stitråkket er en dobbelstrek som følger strekbredden

Prikkene fra v7.9.12 ble «kampesteiner i et steingjerde» så snart strek-skalaen gikk opp: bredden fulgte `--stroke-scale`, mens `stroke-dasharray` sto i faste mm, så en tykkere strek fylte igjen lufta. Stitråkket (ISOM 507) er nå ISOM-ens dobbelstrek — strek, kort luft, strek, lang luft — med rette ender (butt cap), og mønsteret er FAKTORER av den effektive strekbredden: `--w` = 0,1 mm × `--stroke-scale` × «Stier»-slideren, og dasharray er `calc(var(--w) * faktor)` per ledd (ny `lib/strekMonster.js`). Strek og indre gap er ISOM-forholdet (5,5 og 1,4 × bredden). Gruppegapet er økt fra ISOM-ens 4,4 til 24 × bredden: med ISOM-gapet fikk 507 mer blekk per mm enn både 505 og 506, og sti-stigen (tydeligst sti er fastest) ville snudd. 24 holder 507 under 506 og gir fortsatt to hele dobbelstreker på en stibit på 18 m. Hvert kartstil-tema setter faktorene selv (`--iso-507-dash-faktor`, `--iso-507-gap-indre-faktor`, `--iso-507-gap-gruppe-faktor`), så gruppegapet kan finjusteres alene. Faktorene har nye navn med vilje: kart bygget før v7.9.13 har `var(--iso-507-dash, …)` bakt inn og ville lest 5,5 som 5,5 meter, så det gamle navnet får fortsatt mm. «Stier»-slideren setter nå også `--strek-sti`, så mønsteret følger bredden den gir. Stige-testen måler blekk per mm og lengste luft i stedet for streklengde.

I samme leveranse følger ALLE stiplinger den effektive bredden, ikke bare 507: 505/506 (temaenes faktorer), kurvehjelperen 103, 305, overlay-streken på 515 og tunnel-stiplingen på vegene (501–504) og jernbanen. Mønsteret er det samme som før ved nøytral skala — faktorene er de gamle mm delt på bredden — men ved skala 1,6 vokser luft og strek med streken i stedet for å gro igjen. Ingen stipling er lenger bakt i faste mm, og en test holder det slik. Celle- og flis-sømmene ble undersøkt og står urørt: bucketingen tildeler hele features til én celle, så dash-fasen brytes ikke, og naboflisene tegner hver sin way helt.
...
Resten er tonen. Fire steder hevdet eller antydet teksten at Lende er bedre enn andre kartapper, og det skal den ikke: «ikke fordi de har løst noe» var en nedsettelse av et valg som faktisk ER en løsning, «må regne på nytt for hver piksel» var en overdrivelse (faktoren endrer seg underveis langs strekket, ikke per piksel), «Lende gir deg papirkartets geometri uten papirkartets skjevhet» var ren skryt der sannheten er et bytte — papiret trykkes med rutenettets nord opp fordi rutelinjene da er parallelle med arkkanten, og Lende har råd til å la det gå fordi den ikke tegner noe rutenett — og «De fleste turkart lever med det. Lende gjør ikke det» hadde samme form. Avsnittet «Det gjør dem ikke ute av stand til å måle» står uendret; det var riktig fra før.

---

## 2026-09-19 — v7.9.6: Rute, stinett, spor og MCP-verktøy svarer i bakkemeter

v7.9.5 rettet linjalen og målestokken; dette er resten av flaten, og den var større enn de tre postene som sto igjen. Regelen som avgjør hver enkelt endring er den samme overalt: en avstand som RAPPORTERES deles på punktskalaen, en avstand som SAMMENLIKNES MOT GEOMETRIEN gjør det ikke. `route()` er grafens eneste rapporteringsgrense, så det er det eneste stedet k slår inn i ruteren — kantvektene, `snapM`, `gapBridgeM`, `gapObstacleMinM`, `maksHullM` og `minOmveiM` blir stående i rutemeter, fordi de måles mot koordinater som er i rutemeter, og en terskel i bakkemeter mot en geometri i rutemeter er to enheter i samme ulikhet. Rutevalget er per konstruksjon urørt: k er én konstant over arket, og en konstant faktor på alle kandidater endrer ikke hvilken som er minst. `coordinates` er av samme grunn urørt — den som tegner dem eller slår opp i DEM-et trenger rutemeter.

Omregnet er derfor rutelengde og rutekostnad, høydeprofilenes avstandsakse (høydene er loddrette og rører k ikke, så stigningen i prosent blir marginalt brattere — og riktigere, for nevneren var for stor), GPS-sporenes lengde, nærhetsvarselets avlesning, stinett-analysens sti- og koblermeter, tetthet og turlengder, og bruddiagnosens hull og omvei. Forholdstallet mellom hull og omvei regnes fortsatt av de urørte rutemeterne: k stryker mot k, og et hull i bakkemeter ved siden av en omvei i rutemeter er et forholdstall som lyver.

MCP-verktøyene var prioriteten, for de er det som brukes til å teste og feilsøke Lende, og et svar derfra som ikke stemmer med linjalen i appen er verre enn intet svar. Begge flatene er koblet opp — stdio-serveren (`mcp/server.js`) og remote-Workeren (`cloudflare/mcp-worker`) — så `planlegg_rute`, `planlegg_rundtur`, `berik_rute`, `tegn_rute_svg`, `turrapport_svg`, `analyser_stinett`, `finn_stinett_brudd` og `hoydeprofil` svarer identisk og likt med appen. Der arket kan byttes under en åpen økt (Stifinneren, nærhetsvarselet) sendes k inn som en GETTER og ikke som en verdi, og Stifinnerens graf-cache har k i nøkkelen, slik at en flis-forfremmelse ikke serverer forrige senters omregning.
...
**Kritisk sti.** Terreng-først-previewen bygges nå fra probe-DEM-et og ikke fra kjerne-DEM-et. Kjernen venter på kyst-signalet og deretter på en ny WCS-henting i 5/10 m — opptil 4 MB — så «terrenget straks» kom på et kystkart først etter to rundturer til. Kystlinje-proben (~200 B) fyres ved t = 0 i stedet for etter probe-DEM-et; DEM-havflaten er fortsatt et krav, den er bare ikke lenger en startport, og Sjøkart gates på samme promise og starter derfor like tidlig. MapView-chunken (439 kB, 133 kB gzippet) forhåndslastes idet byggingen starter — den lastes ellers først ved `router.push`, altså rett foran det første kartet på første besøk eller etter en ny service-worker-versjon. Og sekundærkildene — Sjøkart, kulturminner, Turrutebasen, N50-sti og N50-areal — er tatt av full-byggets kritiske sti: de får et restbudsjett på fem sekunder målt fra at OSM, DEM og vann er inne. De fyres alle ved t = 0, så når budsjettet starter har de allerede hatt hele Overpass-tida på seg; det biter bare når en tjeneste henger, og det var nettopp der én treg tredjepart holdt hele kartet.

**Ett kart, én worker.** `buildSvgClient` er skrevet om til en ØKT: én worker for hele byggingen i stedet for én per kall. Terreng-først bygger samme ark to ganger, og det kostet to struktur-kloner av DEM-et og to komplette kontur-, stup- og topp-pass over et rutenett som ikke hadde endret seg. Workeren holder nå DEM-et og det DEM-avledede resultatet mellom kallene, og gjenbruken er kallerens beslutning — den vet om kyst-oppgraderingen eller Terrarium-fyllet byttet ut DEM-et. Nedskaleringen og pakkingen av lagrings-DEM-et (`downsampleDem` + `packDem` + `findHighestPoint`) er flyttet inn i workeren og transfereres tilbake i stedet for å kopieres. To unødvendige kopier i konturpasset er også borte: `fillNoData` teller før den kopierer (full WCS-dekning er normaltilfellet, og da var hele kopien bortkastet), og `buildContours` sendte rutenettet gjennom en `Array.from` med begrunnelsen «d3-contour krever Array» — det gjør den ikke. Kontur-utfallet er verifisert byte-identisk før og etter.

**Entryen ligger i minnet.** Kart-entryen var ferdig i minnet når byggingen var over, men MapView leste den ut av IndexedDB igjen, og finalize en gang til — to runder med strukturert kloning av en 1–5 MB SVG-streng og et DEM-buffer for data vi nettopp hadde. Et register med ÉN plass leverer den én gang til `loadMap`; alt annet (gjenåpning, nabofliser, et bygg brukeren navigerte vekk fra) går som før mot IndexedDB.

**Nett-hygiene.** Overpass-kappløpet avgjør vinneren på svar-hodene og avbryter de to taperne der, i stedet for å la alle tre strømme 0,4–5 MB hver før `Promise.any` over `res.json()` kåret en vinner. DEM-hedgen måles på tid til første byte: et stort kyst-DEM bruker lett mer enn fire sekunder bare på kroppen, så med det gamle målet fyrte fallback-endepunktet alltid på nøyaktig de hentingene som var dyrest fra før. NVE Innsjødatabasen ble spurt TO ganger per bygg — `query` og `identify` mot samme server — og identify-svaret ble kastet der query hadde dekning; identify er nå en ekte fallback som bare fyres når query kom tomhendt tilbake, og query ber om de to feltene vi faktisk leser i stedet for hele attributt-tabellen. Til sist caches N50-flisene av service workeren: de er opptil 2,4 MB hver, et 10 km-kart bruker ~3 % av det det laster ned, og GitHub Pages' ti minutters max-age gjorde at neste kart i samme område lastet dem om igjen. De ligger i en uversjonert cache — en deploy skal ikke koste brukeren 100+ MB — og invalideres i stedet på URL-en: klienten henger en manifest-hash på hver flis, så en ny bake er en cache-bom og ikke et gammelt svar.

Rekkefølge-punkt 7 (rasterisert bakgrunn) er forkastet: turkartet er ren SVG, «genetisk» og med vilje.

---

## 2026-09-12 — v7.7.13: død og sovende kode ut av kart-pipelinen

Ryddesteget før ytelsesarbeidet: alt som ikke lenger kalles, og alt som bare kunne nås gjennom en ekvidistanse appen ikke lar deg velge, er borte. Ingenting her lå på kritisk sti — det sto i veien for å lese pipelinen.

Død kode: `waterMaskFromTiles.js` og `waterMaskFromWms.js` (hele moduler), `scripts/build-vardasen-stub.js`, `dem.buildHillshade` (dublett av `hillshade.js`) og `dem.contoursToSvgPaths`, `pathUtils.simplifyVW`/`generalize`, `marineTopology.clipPolygonToSea`/`multiPolygonToPathD`/`ringCentroid`, `demSampling.cropDem`/`sampleGradient`, `n50StiFetcher.fetchN50Sti`, `turrutebasenFetcher.fetchTurruter`, `n50ArealFetcher.nullstillNavnCache`, `mapBuilder.autoMapAFormat`, hillshade-opsjonene `feather`/`vignette`/`decorate` og `'opaque'`-modusen, `*_HIT`-varianten i `stedsmerkeAnimation.js` (en rest etter CurveInvaders), og `knownArea`/`KNOWN_AREAS`/`useReal` i `demFetcher.js` — `fetchDEM` mistet samtidig sitt ubrukte `bbox`-argument. `DEM_TILE_CACHE_ENABLED` var konstant `true`, så AV-grenen og `fetchDEM`-importen i `createMapFlow.js` gikk med den, og `n50ArealStatus` ble sendt inn til `buildSvg` uten at `mapBuilder` noen gang leste den. De fire `buildSvg`-opsjonene ingen kaller varierte (`includeCliffs`, `includeKnauser`, `includeBuildingMass`, `skipContoursIfSynthetic`) er fjernet med grenene sine; den siste er nå oppførselen uansett, altså at et syntetisk DEM aldri får sette kurver på et kart.

Sovende kode: gyldige ekvidistanser er 10/20/25/50 (`equidistanceRules.js`), Fritt lende er 10 og MCP håndhever minimum — så alt som krevde ≤ 5 m var uoppnåelig. Ut går fin-DEM-trappa (`fineDemResFor`, `FINE_DEM_STEPS_M`, `fineInlandTargetResM`) og 10 m-grenen i `demProbeOpplosning`, som nå bare svarer 20 m med mindre kalleren overstyrer — `overstyring`-stien står, den er Fritt lendes. Knaus-deteksjonen (`detectKnauser` + `computeTPI`, som ingen andre brukte) krevde `contourIntervalM === 5` og er borte sammen med knaus-laget i `mapBuilder`; brukerens egne knaus-annoteringer er en annen ting og er urørt. Den gaussiske konturglattingen i `mapBuilder` krevde DEM ≤ 3,5 m, som kyst-oppgraderingen aldri kommer under — `smoothGridGaussian` selv blir stående, for 3D-kurvene (`tour3d/contourLines.js`) bruker den med 15 m. Eldre lagrede kart kan bære 5 m i `meta.equidistance`; de klemmes nå til minst 10 der de leses inn i et nytt bygg (`useMapExtend`), så ingen gren trengs for dem.
...
## 2026-09-11 — v7.7.9: patch og minor i nettleser-treet tatt inn, det brytende holdt igjen

Rot-treets oppdateringer innenfor deklarerte spenn er tatt inn selv, slik CLAUDE.md foreskriver framfor å merge Dependabot rått: vue 3.5.41 → 3.5.42, vue-router 5.2.0 → 5.3.1, @napi-rs/canvas 1.0.7 → 1.0.9, playwright 1.62.1 → 1.63.0 og vite 8.2.2 → 8.3.0. To av robotens PR-er (#305 vue-router, #306 @napi-rs/canvas) var allerede oppfylt av treet og lukkes som superseded. Det som IKKE er med er like bevisst: three 0.185 → 0.186 og `agents` 0.21 → 0.23 ser ut som minor, men for `0.x`-pakker er minor-feltet det brytende — `agents` bærer dessuten hele MCP-Workerens handler-valg og skal gjennom `mcp:protokoll`-gaten i sin egen leveranse — og vitest 5 og actions/cache v4 → v6 er ekte majors, som CLAUDE.md vil ha alene fordi det er de som trenger en ekte gjennomgang. Lockfila er verifisert i begge retninger etter v7.7.8: ingen `version`-felt i utakt med sin egen `resolved`-URL, og alle 43 plattform-spesifikke valgfrie binærene står igjen, så fila installerer fortsatt på annet enn en Linux-runner.

---

## 2026-09-11 — v7.7.8: lockfilas tretten gale versjonsfelt, rettet kirurgisk

Tretten noder i rot-lockfila sto med appens egen versjon fra den gang (`3.0.17`) i stedet for pakkas — restene av en global søk-og-erstatt under en versjons-bump, som traff nettopp de pakkene hvis ekte versjon begynner på `3.0.`. Installasjonen var aldri gal, for npm installerer fra `resolved` + `integrity`, men `audit`, `outdated` og dedupe leser `version`, og feltet er trolig kilden til de sporadiske 400-ene («Invalid package tree») fra audit-endepunktet. Notatet i CLAUDE.md sa fem noder og foreskrev `rm package-lock.json && npm install`; begge deler var feil. Oppskriften fikser ingenting og gjør skade: npm skriver en skjult lockfile i `node_modules/` som bar samme korrupsjon, og med den på plass tar npm en snarvei forbi hele registeroppslaget — den nye fila fikk de gale tallene tilbake og mistet i tillegg 52 noder, nemlig hver plattform-spesifikke valgfrie binær, altså en lockfile som ikke installerer på annet enn en Linux-runner. Rettingen er i stedet kirurgisk: feltet satt fra pakkas egen `package.json` på disk, med versjonen i `resolved`-URL-en som påkrevd kryssjekk, og en diff på nøyaktig 13 linjer og ingenting annet. `npm ci` gjenskaper treet uendret, og notatet i CLAUDE.md er rettet med målingene bak seg.
...
Øyene forsvant når man valgte en Stemning, og innsjøen ble heldekkende. Naturreservat-overlayen (ISOM 520) males ETTER vann, og katalogen gir den derfor alfa — men alle seks monokrom-temaene overstyrte den med et OPAKT hex. Resultatet var at hele verneområdet ble malt over med én flat farge: innsjøen, øy-hullene og strandlinja med. Det ble målt på Nesøytjern i Asker, der diagnose-modus viste at øya var ett lag mens vannet rundt var to. Vasken er nå temaets egen 520-KANT med alfa — kanten er allerede valgt for å leses på det temaet, og ramme og vask i samme blekk leses som ett merke — og en test holder alfaen på plass i alle temaer.

Snarvei-radens «Mer / Mindre» var en knapp med ikon og etikett i SAMME flex-rad som funksjonene: formet som en snarvei, plassert som en snarvei, men den eneste knappen i raden som ikke gjorde noe med kartet. Den er byttet med appens grå drawer-håndtak, bunnplassert og midtstilt som i punkt-arket — dra ned for å folde ut alle snarveiene, dra opp for å legge sammen. Et trykk gjør det samme, så gesten aldri er eneste vei. «Sorter snarveier» er fristilt igjen: sin egen svarte, midtstilte knapp under den åpne raden, uten ikon.

Strek og relieff er flyttet dit de hører hjemme. De sto som gruppe-piller med tannhjul på linja raden åpnet, med et delt bunn-ark bak seg — altså en tredje klasse kontroll midt i en rad som ellers bare bærer FUNKSJONER, og kartets uttrykk ble stilt inn to steder. Nå er de to seksjoner nederst i Innstillinger → Kartstil, sammen med tema, lag og sti-farge. Pillene, tannhjulene, gruppe-pille-formen, knott-buen som viste nivået og hele `FabSettingsPanel` er slettet — ikke flyttet. Nivået står som et tall ved en slider i stedet for som en bue man skal tolke, og hele skalaen er synlig på én gang i stedet for ett hakk med wrap.

Til slutt følger snarveiene hovedmenyens tekststørrelse, opp til 200 %. Resten av appen har gjort det lenge; raden var det ene stedet valget ikke slo gjennom — altså nettopp den flata man leser mens man går. `zoom` settes på hver KNAPP og ikke på raden, fordi en zoomet rad også skalerer sitt eget gap og sin egen polstring, og da måles et budsjett i én enhet mot knapper i en annen. Ved 200 % får tre knapper plass på en linje i stedet for seks, og resten ligger ett drag unna — det er meningen.
...
Første pass for en flis males ABSOLUTT og ikke som en diff, og det er ikke en detalj: en diff mot et tomt forrige-sett kan per definisjon bare vise, aldri skjule, så en flis som festes MENS man står dypt inne i kartet ville stått fullt tegnet til brukeren beveget seg. Aktiv flis slipper unna det fordi dens første pass alltid skjer på oversikts-zoom, der alt er synlig uansett; en spøkelses-flis har ingen slik åpning.

Indeksen kan ikke bo sammen med den aktive: spøkelsene festes og demonteres uavhengig av kart-lastene som bygger `cullIndex`. Den bor derfor i et `WeakMap` nøklet på selve flis-noden, så den overlever en demontering og dør med noden, med en generasjons-teller som invaliderer det hurtigbufrede synlighets-settet når `resetViewportCull` eller utvikler-bryteren vasker `vp-cull` ut av DOM-en. Forskyvningen leses av attributtene `buildGhostSvg` faktisk skrev (`ghostFlisRom` i `lib/viewportCull.js`, ren og enhetstestet) framfor å anta blø-bredden, så endrer noen blø-sonen følger regnestykket med. Og feste-siden VARSLER cullingen (`onFesteEndret`): `useViewportCull`s egen watcher er registrert først i MapView, så i samme tikk ville cull-passet kjørt før feste-passet og en fersk flis stått ucullet til neste pan.

Eksport-kontrakten er urørt. `.vp-cull` bor i MapViews u-scopede stilark og aldri i kartets eget, og de fem `medAlleSpokelserFestet`-konsumentene leser attributter eller baker flisene med sitt eget stilark — en klasse på en spøkelses-rot er usynlig for dem alle. Utvikler-fanen har fått en rad som viser hvor mange fliser som er skjult i sin helhet og per-element-tallet for dem som ligger delvis inne, fordi tiltaket skal kunne vurderes i felt i stedet for gjettes på, og røyktesten har en egen sjekk som fabrikkerer to nabofliser med nøyaktig `buildGhostSvg`s attributter og krever at den fjerne culles hel mens den nære culles innvendig. Sjekken fant nettopp første-pass-feilen over, og den måtte selv rettes to ganger for å måle noe i det hele tatt: kartet panorerer bare på `touchstart`, så en mus-drag flytter ingenting, og en tur ut og inn med hjulet ender der den startet — hysteresen måler mot forrige KJØRTE pass, og de mellomliggende tikkene spises av debouncen.

---

## 2026-09-08 — v6.5.72: Kartdata som gjelder arket, og tre layout-flush-er ut av gest-stien

«Mine kart» og info-arket i turkartet sier nå det samme om et kart, og begge tallene beskriver ARKET: bredde × høyde i km og datamengden i MB, summert over aktiv flis og naboene. To feil så begge ut som at tallene ikke oppdaterte seg. Et ark på én flis falt tilbake på `halfKm` og ble skrevet som et kvadrat, så et 10 × 20 km-kart sto som «10,0 × 10,0 km» selv om `utmBbox` hadde de ekte målene; og MB-tallet var midtflisas `sizeBytes` ved siden av arkets mål, altså to ulike ting i samme setning, som sto stille mens brukeren utvidet kartet. Regelen bor nå i `lib/kartStorrelse.js` (ren og enhetstestet), `arkExtentFor` summerer byte for flisene som teller med, og mosaikk-modellen (`ghostRects`) bærer byte per flis — så «Kartdata» i info-arket er en computed som følger en utvidelse uten et eget oppdateringskall.

Ytelse: tre tvungne layout-lesninger er tatt ut av pointermove-stien. `getBoundingClientRect()` tvinger nettleseren til å flushe stil og layout for et dokument kart-transformen nettopp skitnet, og den ble betalt tre ganger per touchmove — i `clampPan` (som nå leser MapViews eksisterende `wrapperSize`, matet av ResizeObserver), i `useMapExtend.transformView` (som cacher rekta gjennom en gest; wrapperen kan ikke flytte seg mens fingrene er nede), og i `positionContextPin`, der gaten sto ETTER rekt-lesningen så den flushet layout også når kontekstmenyen var lukket. I tillegg gjøres stiplede streker solide under gest med ÉN CSS-regel i stedet for en inline stil per path: den gamle veien gikk gjennom hele dokumentet og skrev titusener av stiler både ved gest-start og ved gjenopprettingen, på nøyaktig den stien som skal være billig. Som bieffekt treffer regelen nå også en SVG som byttes ut midt i en gest — den fikk `.is-zooming`-klassen, men aldri de inline stilene.
...
«Start GPS» var den ene bryteren man vil nå mens man går, og lå bak et trykk og en skuff som dekker kartet. Den lever videre som en pin-knapp rett over kompasset, i samme 48 px-skive og med samme regel om at fargen leser mot ARKETS valør og ikke mot app-chromet. Tilstanden bæres av farge og av `aria-pressed`: på er GPS-prikkens egen blå, slik at knappen forklarer prikken den slår på, og av er en nøytral grå — ikke en dempet blå, for forskjellen skal kunne ses uten å huske hvordan på ser ut. `stopPositioning()` i `useGpsSpor` slår av begge, fordi på slår på begge; uten det ville et kart som dreier seg med telefonen ikke hatt noen vei ut etter at kompass-knappen forsvant.

Underveis kom en tredje ting fram som ikke var ny, men som den ekstra knappen gjorde synlig: navigasjonssøyla på høyre kant rant ut under skjermkanten på en liggende telefon, og `.kart-ui` sto med `overflow-hidden` — som klipper, men fortsatt lager en rulleboks. Ga nettleseren fokus til noe utenfor kanten, rullet den hele kart-chromet 112 px opp, og skuffens fane-rad havnet over skjermtoppen. Overlegget er nå `overflow-clip`, som ikke kan rulles i det hele tatt, og søyla har fått et høydetak slik at den ruller i seg selv i stedet. Røyktestens liggende-sjekk måler begge deler.

---

## 2026-09-06 — v6.5.67: kompasset er sin egen flate, og rewind-knotten har fått søskenflaten sin

Kompasset i berøringssøyla hadde en knappe-bakgrunn under seg — en sirkel inni en sirkel — og valøren den skiftet på var UI-temaets. Begge deler er rettet: skiva i selve nåla er nå flaten man sikter på, og den leser mot KARTET og ikke mot chromet, fordi knappen står rett på arket (`mork`-propen mates av `isDark` i MapView, altså arkets eget bakgrunnsvalør). Skiva er 48 px, nøyaktig like bred som pilla med pluss og minus rett over, så de to leser som ett søyle-ledd og ikke som en stor og en liten knapp; trefflaten er dermed skiva selv, og aria-etiketten med gradtallet står uendret. Ikonet i den øverste FAB-knotten var 3D-ens «Oversikt»-glyf limt rett inn i boksen — full bredde, strek 2, ingen ramme — mens strek- og relieff-knottene begge er en liten glyf inne i en tynn bue. Glyfen er nå fire hjørner som rammer inn et utsnitt, med tynnere strek og god luft inn til en HEL ring: de to andre knottene måler et nivå, denne har ingen, og en bue med gap ville lovet en fylling som aldri kommer. Hjørnene sier også bedre hva knotten gjør enn en rewind-pil gjorde — den rammer inn arket slik det åpner, den spoler ikke noe tilbake i tid.
