...
//
// Å DREIE MERKET MOT SANN NORD ER PRØVD I FELT OG FORKASTET (v6.5.60 → v6.5.61).
// Ikke ta det opp igjen uten nye argumenter. Argumentet FOR var godt på papiret:
// arket står i hvile på `nordRotasjon` (v6.5.59), altså litt på skrå, og da peker
// pila merket «Nord i lende» langs RUTENETTET og ikke mot sann nord — 1,5° i Oslo,
// 19,9° i Kirkenes. Fradraget var fem linjer og virket nøyaktig som beskrevet.
//
// Det som avgjorde var å se det på en telefon: merket sluttet å flukte med kanten
// det sitter på, og et håndtak som ligger på skrå av sin egen arkkant leses som en
// feil i kartet lenge før noen leser det som en kompassretning. Trekant-geometrien
// er tegnet for å flukte (v5.25.5–6), og hjørnemerket for å ha beina parallelle med
// arkets to kanter. Merket er altså et ARKMERKE: det sier hvilken flis et trykk
// bygger, og flisene ligger i rutenettet. Kompasset er kompassnåla og rosa, som
// begge trekker korreksjonen fra og fortsatt gjør det.
...
export function useMapExtend({
  ...
  function autoMapBuildOpts(centerSvg) {
    ...
    return {
      ...
      // Klem til minst 5 m: gyldige ekvidistanser er 5/10/20/25/50
      // (equidistanceRules), men et kart lagret før v6.5.76 kan bære 2,5 m.
      // En 5 m-flis skal gi 5 m-naboer, ellers møtes to kurvetettheter i sømmen.
      equidistanceM: Math.max(5, m.equidistance ?? 20),
      navn: `Tur ${stamp}`,
