...
export function buildIsomCss(catalog = isomCatalogDefault, patternIds, options = {}) {
  ...
  // Hjelpekurver (ISOM 103) vises når laget er bygget (bruker-bryter «Hjelpekurver»).
  // De er tynne/stiplede så de leser som en bratthets-/form-indikasjon uten å
  // overdøve hovedkurvene — ingen zoom-gating (bryteren styrer synligheten).
  ...
  // `color`. Fargetabellen ligger i poiColors.js fordi 3D-nålene bruker de samme
  // verdiene; ville de blitt skrevet av her, kunne de to drevet fra hverandre.
  rules.push(...brukerminneColorRules(root))
