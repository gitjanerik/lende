...
// FANA VISER FIRE AV NI STREK-GRUPPER (v7.8.12), og lista i `strokeOverrides`
// er MED VILJE ikke rørt: den er kontrakten MCP-verktøyet `juster_kart`
// validerer mot, den kartstilene setter sine egne vekter i (Print ganger
// `bygg`), og den `buildStrokeOverrideCss` bygger CSS av. Det som ble skrellet
// bort er en RAD MED SKYVEKNAPPER, ikke en funksjon: stup, naturreservat-
...
// SKRIFTEN PÅ KART-NAVN KOM HIT FRA «STEMNING» (v7.8.12). Nedtrekket sto i en
// fane som ellers bare var tema-knapper, og det er ikke en stemning — det er
// hvordan kartet SER UT, altså nøyaktig det denne fana eier. Prøven under er
// ikke pynt: font-par leses ikke av navnet sitt.
...
<template>
  <div>
    ...
    <!-- ── Skrift (v7.8.12, flyttet hit fra «Stemning») ─────────────────
         Land = sans (bebyggelse/topp/område), vann = kursiv serif. Byttes
         live. Prøven under står i parets EGNE familier: et font-par leses ikke
         av id-en sin, og et nedtrekk med seks navn uten form er seks gjetninger. -->
    <div class="mt-5 pt-4 border-t border-ink/10">
      <div class="text-[11px] font-semibold text-ink-3 uppercase tracking-wide mb-1.5">
        Skrift på kart-navn
      </div>
      <div class="rounded-lg bg-ink/5 px-3 py-2.5">
    ...
    <div data-stil-seksjon="strek" class="mt-5 pt-4 border-t border-ink/10">
      ...
      <div v-for="g in strekGrupper" :key="g.id"
